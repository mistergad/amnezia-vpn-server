...
Bootstrap автоматически устанавливает Docker, PostgreSQL, Caddy и systemd-службу, собирает официальный контейнер AWG3, создает БД, секреты и HTTPS-конфигурацию. Серверный интерфейс `awg0` получает MTU `1280` при деплое и каждом запуске контейнера. Все клиентские `.conf`, QR-коды и текстовые ключи получают `MTU = 1200`; это значение задается через `AWG_MTU`. Для каждого устройства по внутреннему VPN-IP настраиваются лимиты `10 Мбит/с` на загрузку и `8 Мбит/с` на отдачу; значения можно изменить параметрами `--download-limit-mbps` и `--upload-limit-mbps`. Скрипт можно запускать повторно: существующий AWG3-контейнер `amnezia-awg2` не пересоздается, его peer-ключи сохраняются, а ограничения синхронизируются со всеми живыми peer. Историческое имя контейнера сохраняется самим официальным клиентом и не означает версию протокола.
