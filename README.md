...
Рабочий MVP сервиса подписки на VPN: администратор управляет балансами клиентов и устройствами, а сервис создает отдельную конфигурацию AmneziaWG для каждого устройства.
...
- изменение клиентского баланса доступно только администратору;
- выдача `.conf`, QR и гостевого текстового `vpn://`-ключа после активации администратором;
...
- тесты полного сценария «регистрация → активация администратором → ключ → отзыв».
...
временно удаляет peer-ключи из AWG 3.1, но сохраняет их конфигурации, IP и статус
приостановки. После следующего пополнения баланса администратором те же ключи автоматически
возвращаются в AWG 3.1 и снова работают без повторного импорта на устройствах.
...
опросами AWG 3.1. Фоновый интервал по умолчанию — 60 секунд; кнопка «Обновить
...
Для локального запуска оставьте `VPN_BACKEND=mock`, затем:

```bash
uvicorn app.main:app --reload
```

Откройте `http://localhost:8000`. Администратор создается из `ADMIN_EMAIL` и `ADMIN_PASSWORD` при первом старте; только он может назначать клиентам баланс.
...
### Одноразовый переход с AWG 2/3.0 на AWG 3.1

Если старые аккаунты и ключи не нужны, выполните на VPS из обновлённого
репозитория:
...
Флаг `--reset-for-awg31` безвозвратно удаляет PostgreSQL-базу `amnezia` вместе
со всеми клиентами, балансами, платежами и ключами, удаляет старый VPN-контейнер,
инвалидирует старые веб-сессии и разворачивает чистый AWG 3.1. Администратор будет
создан заново из текущих `ADMIN_EMAIL`/`ADMIN_PASSWORD`; актуальные реквизиты
останутся в `/root/amnezia-service-credentials.txt`. Этот флаг нельзя добавлять
в обычную команду последующих обновлений.

Без этого флага deploy откажется использовать найденный AWG 2/3.0-контейнер,
чтобы панель не начала выдавать несовместимые AWG 3.1-конфигурации. Старые
импортированные ключи несовместимы с включенными параметрами 3.1, поэтому после
перехода устройства нужно подключить заново.

Для Android используйте официальный pre-release
[AmneziaWG `v3.1.20260814`](https://github.com/amnezia-vpn/amneziawg-android/releases/tag/v3.1.20260814)
или
более новый клиент с поддержкой `RandomTrailers` и `DisableCookies`. Стабильный AmneziaVPN
`5.0.0.5` вышел раньше AWG 3.1 и этих двух параметров не поддерживает; до выхода
следующего стабильного AmneziaVPN подключайте `.conf`/QR через отдельное
приложение AmneziaWG. Текстовый `vpn://`-ключ уже имеет формат
`protocol_version = 3.1`, но требует более новую сборку AmneziaVPN.
...
## Подключение существующего AmneziaWG 3.1
...
5. В `/etc/amnezia-service.env` задайте:
   ...
   Если `AWG_CONFIG_PATH` не существует на хосте, сервис автоматически читает параметры J/S/H/I и AWG 3.1 через `awg showconf` внутри контейнера. Конфигурация обязана содержать включенные `RandomTrailers` и `DisableCookies`. Ключ `-i` у `docker exec` обязателен: preshared key передается процессу через stdin и не попадает в аргументы командной строки. Для ручного подключения rate limit также установите `iproute2` в AWG-контейнер, скопируйте `deploy/awg2-traffic-limit.sh` в `/opt/amnezia/traffic-limit.sh` и разрешите эту команду через sudoers. Автоматический bootstrap выполняет эти действия сам.
...
- проверить требования законодательства вашей юрисдикции к VPN и персональным данным;
...
app/
  ...
  models.py                  пользователи, баланс, подписки, peer-ключи
