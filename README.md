...
временно удаляет peer-ключи из AWG3, но сохраняет их конфигурации, IP и статус
приостановки. После следующего успешного пополнения те же ключи автоматически
возвращаются в AWG3 и снова работают без повторного импорта на устройствах.
...
опросами AWG3. Фоновый интервал по умолчанию — 60 секунд; кнопка «Обновить
...
Bootstrap автоматически устанавливает Docker, PostgreSQL, Caddy и systemd-службу, собирает официальный контейнер AWG3, создает БД, секреты и HTTPS-конфигурацию. Серверный интерфейс `awg0` получает MTU `1200` при деплое и каждом запуске контейнера. Все клиентские `.conf`, QR-коды и текстовые ключи также получают `MTU = 1200`; это значение задается через `AWG_MTU`. Для каждого устройства по внутреннему VPN-IP настраиваются лимиты `10 Мбит/с` на загрузку и `8 Мбит/с` на отдачу; значения можно изменить параметрами `--download-limit-mbps` и `--upload-limit-mbps`. Скрипт можно запускать повторно: существующий AWG3-контейнер `amnezia-awg2` не пересоздается, его peer-ключи сохраняются, а ограничения синхронизируются со всеми живыми peer. Историческое имя контейнера сохраняется самим официальным клиентом и не означает версию протокола.

Серверные скрипты AWG3 перенесены из `amnezia-vpn/amnezia-client` и зафиксированы на commit `dcf53b989e684a2e3e3f7f5c090001fb2def73b9`; серверный образ закреплен на `amneziavpn/amneziawg-go:3.0.20260805` вместе с immutable digest. Происхождение и GPL-3.0 лицензия описаны в `deploy/vendor/amnezia-client/UPSTREAM.md`. Перед созданием контейнера bootstrap проверяет поддержку AWG3 одновременно в backend и `awg-tools`; это убирает интерактивную установку через десктопный клиент и не позволяет будущему изменению репозитория незаметно поменять deploy.

### Одноразовый переход с AWG2 на AWG3
...
Флаг `--reset-for-awg3` безвозвратно удаляет PostgreSQL-базу `amnezia` вместе
со всеми клиентами, балансами, платежами и ключами, удаляет старый VPN-контейнер,
инвалидирует старые веб-сессии и разворачивает чистый AWG3. Администратор будет
...
Без этого флага deploy откажется использовать найденный AWG2-контейнер, чтобы
панель не начала выдавать несовместимые AWG3-конфигурации. Для подключения новых
ключей требуется AmneziaVPN 5.0.0.5 или новее.
...
## Подключение существующего AmneziaWG 3
