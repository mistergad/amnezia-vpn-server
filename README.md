...
- автоматическая выдача первого `.conf` после оплаты, QR и дополнительные ключи;
...
- автоматическое отключение ключей по окончании подписки;
...
Расход учитывается посекундно. Фоновая сверка обновляет баланс и отзывает peer-ключи после его исчерпания. Старые оплаченные периоды при обновлении автоматически конвертируются в баланс без потери оставшегося времени.
...
В режиме IP Caddy использует локальный центр сертификации: трафик остается
зашифрованным, но браузер покажет предупреждение, пока его корневой сертификат
`/var/lib/caddy/.local/share/caddy/pki/authorities/local/root.crt` не импортирован
в доверенные центры сертификации администраторского устройства. Для публичного
сервиса домен с обычным публично доверенным сертификатом предпочтительнее.
...
Для резервной копии нужны PostgreSQL, `/etc/amnezia-service.env` (особенно `ENCRYPTION_KEY`) и `/opt/amnezia/awg/awg0.conf` из контейнера. HTTPS-сертификат Caddy получит автоматически, когда домен резолвится на VPS и TCP `80/443` доступны извне.
